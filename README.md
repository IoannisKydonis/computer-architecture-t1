...
- MinorCPU: Είναι ένα in-order μοντέλο CPU με σταθερού μεγέθους pipeline το οποίο αποτελείται από 4 στάδια (Fetch1, Fetch2, Decode, Execute) αλλά έχει αρκετά παραμετροποιήσιμη συμπεριφορά. Αυτό του δίνει τη δυνατότητα να προσομοιώνει ικανοποιητικά πραγματικούς επεξεργαστές αποκτώντας παρόμοια χαρακτηριστικά με αυτούς.
